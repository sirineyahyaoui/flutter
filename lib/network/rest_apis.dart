...
import 'package:booking_system_flutter/utils/one_signal_utils.dart';
...
Future<void> logoutApi() async {
  ...
  String firebaseToken = appStore.firebaseToken.isNotEmpty ? '&firebase_token=${appStore.firebaseToken}' : '';
  return await handleResponse(await buildHttpResponse('logout$playerId$firebaseToken', method: HttpMethodType.GET));
}
...
Future<ConfigurationResponse> getAppConfigurations({bool isCurrentLocation = false, double? lat, double? long}) async {
  try {
    ConfigurationResponse? data =
        ConfigurationResponse.fromJson(await handleResponse(await buildHttpResponse('configurations?is_authenticated=${appStore.isLoggedIn.getIntBool()}', method: HttpMethodType.GET)));
    ...
    data.configurations.validate().forEach((data) {
      ...
      if (data.value.validate().isNotEmpty && data.key == generalSettingKey) {
        setGeneralSetting(GeneralSettingModel.fromJson(jsonDecode(data.value.validate())));
      }
    ...
    if (data.generalSetting != null) {
      appStore.setHelplineNumber(data.generalSetting!.helplineNumber.validate(), isInitializing: true);
      appStore.setInquiryEmail(data.generalSetting!.inquriyEmail.validate(), isInitializing: true);
    }
    ...
    if (data.otherSettings != null) {
      ...
      await setValue(FIREBASE_NOTIFICATION_STATUS, otherSetting.firebaseNotification.validate().getBoolInt());
    ...
    if (data.countryList != null && data.countryList.validate().isNotEmpty) {
      if (data.countryList.validate().first.currencyCode.validate() != appStore.currencyCode) appStore.setCurrencyCode(data.countryList.validate().first.currencyCode.validate());
      if (data.countryList.validate().first.countryId.validate().toString() != appStore.countryId.toString())
        appStore.setCurrencyCountryId(data.countryList.validate().first.countryId.validate().toString());
      if (data.countryList.validate().first.currencySymbol.validate() != appStore.currencySymbol) appStore.setCurrencySymbol(data.countryList.validate().first.currencySymbol.validate());
    }

    otherSettingStore.setPostJobRequestEnable(data.postJobRequest.validate());
    setValue(IS_ADVANCE_PAYMENT_ALLOWED, data.isAllowAdvancePayment.validate().getBoolInt());
...
void _performAdditionalProcessing(DashboardResponse dashboardResponse) async {
  ...
  if (dashboardResponse.countryList != null && dashboardResponse.countryList.validate().isNotEmpty) {
    if (dashboardResponse.countryList.validate().first.currencyCode.validate() != appStore.currencyCode)
      appStore.setCurrencyCode(dashboardResponse.countryList.validate().first.currencyCode.validate());
    if (dashboardResponse.countryList.validate().first.countryId.validate().toString() != appStore.countryId.toString())
      appStore.setCurrencyCountryId(dashboardResponse.countryList.validate().first.countryId.validate().toString());
    if (dashboardResponse.countryList.validate().first.currencySymbol.validate() != appStore.currencySymbol)
      appStore.setCurrencySymbol(dashboardResponse.countryList.validate().first.currencySymbol.validate());
  }
